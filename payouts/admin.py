...
class PayoutReferralInline(admin.TabularInline):
    ...
    fields = ('referral', 'amount', 'created_at')
    can_delete = False
...
class EarningsAdmin(admin.ModelAdmin):
    ...
    referral_link.allow_tags = True

    def mark_as_available(self, request, queryset):
        updated = 0
        for earning in queryset.filter(status=Earnings.Status.PENDING):
    ...
    def mark_as_processing(self, request, queryset):
        updated = 0
        for earning in queryset.filter(status=Earnings.Status.AVAILABLE):
    ...
    def mark_as_paid(self, request, queryset):
        updated = 0
        for earning in queryset.filter(status=Earnings.Status.PROCESSING):
